...
import { Device, SimpleSubscription, SimpleSubscriptionJson, Subscription } from "../types";
...
export function getSimpleSubscriptionFromEdge(edges: Edge[], edgeId: string): SimpleSubscription | null {
  if (edges === undefined) {
    console.log('undefined');
    return null;
  }
  ...
  return (
    {
      transmitter: {
        deviceName: properEdge.source,
        channelName: properEdge.sourceHandle.slice(3)
      },
      receiver: {
        deviceName: properEdge.target,
        channelName: properEdge.targetHandle.slice(3)
      }
    })
}
